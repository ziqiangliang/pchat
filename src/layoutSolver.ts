...
export class LayoutSolver {
  ...
  private applyGroupConstraint(
    ...
    centerX /= groupNodes.length;
    centerY /= groupNodes.length;
  ...
  private applyCenterConstraint(
    ...
    if (centerNodes.length === 1) {
      ...
      const pos = positions.get(nodeId);
      const centerX = pos?.x || 400;
      const centerY = pos?.y || 250;
      positions.set(nodeId, { x: centerX, y: centerY });
      return;
    }
  ...
  private applyHorizontalConstraint(
    ...
    const spacing = 120;
    const startX = 400 - (nodes.length - 1) * spacing / 2;
    const avgY = nodes.reduce((sum, nodeId) => {
      const pos = positions.get(nodeId);
      return sum + (pos?.y || 0);
    }, 0) / nodes.length;
  ...
  private applyVerticalConstraint(
    ...
    const spacing = 100;
    const startY = 250 - (nodes.length - 1) * spacing / 2;
    const avgX = nodes.reduce((sum, nodeId) => {
      const pos = positions.get(nodeId);
      return sum + (pos?.x || 0);
    }, 0) / nodes.length;
  ...
  private flowLayout(nodes: Node[], edges: Edge[]): Map<string, Position> {
    ...
    levelGroups.forEach((nodeIds, level) => {
      ...
      nodeIds.forEach((nodeId, index) => {
        const x = 100 + index * spacing + (levelGroups.size - 1) * spacing / 2;
  ...
  private dataLayout(nodes: Node[]): Map<string, Position> {
    ...
    nodes.forEach((node, index) => {
      const value = parseFloat(node.label) || Math.random() * maxHeight;
      const height = Math.max(20, value);
      positions.set(node.id, {
        x: 100 + index * barWidth,
        y: baseY - height
      });
    });
