...
class TTSManager {
  ...
  public async speak(text: string, options?: { rate?: number }): Promise<void> {
    try {
      if (this.currentProvider === 'aliyun' && aliyunTTSService.isSupported) {
        ...
        const ttsService = (await import('./ttsService')).ttsService;
        if (options?.rate) {
  ...
  public pause(): void {
    if (this.currentProvider === 'aliyun') {
      ...
      console.warn('Native TTS pause not implemented');
    }
  }
  ...
  public resume(): void {
    if (this.currentProvider === 'aliyun') {
      ...
      console.warn('Native TTS resume not implemented');
    }
  }
  ...
  public stop(): void {
    if (this.currentProvider === 'aliyun') {
      ...
      console.warn('Native TTS stop not implemented');
    }
  }
}
